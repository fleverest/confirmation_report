...
* Take a relatively small sample of ballots
* *Decide* between $H_0$ and $H_1$
* ($H_0$ and $H_1$ are not switched)
...
text(0, 0.5, expression(H[1]), cex=2)
text(0, 0.15, expression(H[0]), cex=2)
```


## Sample coherence

An audit is sample-coherent if we certify only when sample-proportions support reported outcome.
...
* Define a prior on $H_0\cup H_1$ (Dirichlet)
...
Bayesian audits with "symmetric" Dirichlet prior with $\kappa\geq0.5$ are sample-coherent.
...
tibble(kappa=x, risk=(1-x)^2) |>
  ggplot(aes(x=kappa, y=risk)) +
  geom_line() +
  xlab(expression(kappa)) +
  ylab("Worst-case Risk") +
...
* `test_smart` as a unified sequential testing library on which to base all new sequential tests
* [Documentation website started](https://fleverest.github.io/testsmart/)


## Optimising Supermartingale Intersection Tests

::: {#fig-supermartingale}
![](./media/supermartingales.png)
